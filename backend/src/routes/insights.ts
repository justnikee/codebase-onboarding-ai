...
import { flattenFileTree } from "../utils/helpers.js";
...
router.get("/:contextId/code-metrics", async (req: Request, res: Response) => {
  try {
    ...
    const suggestions =
      await codeAnalyzerService.generateImprovementSuggestions(
        metrics,
        smells,
        patterns,
      );
    ...
    try {
      ...
      if (!type || type === "dependency-graph") {
        result.dependencyGraph =
          await visualizationService.generateDependencyGraph(
            context.fileStructure,
            context.repoUrl,
          );
      }
      ...
      if (!type || type === "architecture") {
        result.architecture =
          await visualizationService.generateArchitectureDiagram(
            context.fileStructure,
      ...
      if (!type || type === "complexity") {
        result.complexity =
          await visualizationService.generateComplexityVisualization(
            context.fileStructure,
          );
      }
 ...
 */
router.get(
  "/:contextId/contribution-opportunities",
  async (req: Request, res: Response) => {
    try {
      const { contextId } = req.params;
      ...
      const opportunities =
        await recommendationService.findContributionOpportunities(
          context.fileStructure,
          context.techStack.languages,
          smells,
        );
 ...
 */
router.get(
  "/:contextId/learning-recommendations",
  async (req: Request, res: Response) => {
    try {
      const { contextId } = req.params;
      ...
      const recommendations =
        await recommendationService.generateLearningRecommendations(
          context.techStack.languages,
          known,
          context.fileStructure,
        );
 ...
 */
router.get(
  "/:contextId/tool-recommendations",
  async (req: Request, res: Response) => {
    try {
      const { contextId } = req.params;
...
router.get("/:contextId/summary", async (req: Request, res: Response) => {
  try {
    ...
    const [metrics, patterns, smells, heatmap, recommendations] =
      await Promise.all([
        codeAnalyzerService.analyzeCodeMetrics(
          context.repoUrl,
          context.fileStructure,
        ),
        codeAnalyzerService.detectArchitecturePatterns(context.fileStructure),
        codeAnalyzerService.detectCodeSmells(
          context.repoUrl,
          context.fileStructure,
        ),
        visualizationService.generateCodeHeatmap(context.fileStructure),
        (async () => {
          const m = await codeAnalyzerService.analyzeCodeMetrics(
            context.repoUrl,
            context.fileStructure,
          );
          const p = await codeAnalyzerService.detectArchitecturePatterns(
            context.fileStructure,
          );
          const s = await codeAnalyzerService.detectCodeSmells(
            context.repoUrl,
            context.fileStructure,
          );
...
/**
 * GET /api/insights/:contextId/first-tasks
 * Generate a "getting started" first task plan for new developers
 */
router.get("/:contextId/first-tasks", async (req: Request, res: Response) => {
  try {
    const { contextId } = req.params;
    ...
    // Flatten file tree to a simple list of paths
    const allFiles = flattenFileTree(context.fileStructure)
      .filter((f) => f.type === "file")
      .map((f) => f.path);

    const plan = await watsonxService.generateFirstTasks({
      repoName: context.metadata.name,
    ...
    console.error("Error generating first tasks:", error);
    res.status(500).json({
      error: "Failed to generate first tasks",
      details: error instanceof Error ? error.message : "Unknown error",
    });
  }
});
...
/**
 * GET /api/insights/:contextId/graph
 * Get a real file-level dependency graph generated from the analyzed repo
 */
router.get("/:contextId/graph", async (req: Request, res: Response) => {
  try {
    const { contextId } = req.params;
    const context = await contextService.getContext(contextId);
    if (!context) {
    ...
    const graph = visualizationService.generateFileGraph(context.fileStructure);
