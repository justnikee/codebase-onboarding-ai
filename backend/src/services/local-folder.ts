...
class LocalFolderService {
  ...
  private readonly MAX_FILES = 500; // local FS — no API rate limits, scan deeply
  ...
  async analyzeFolder(folderPath: string): Promise<LocalAnalysisResult> {
    ...
    console.log(
      `[LocalFolder] Analysis complete: ${totalFiles} files, ${(totalSize / 1024).toFixed(2)} KB`,
    );
  ...
  private async buildFileStructure(
    ...
    if (depth > 10) return []; // max depth for local uploads
    if (collected.count >= this.MAX_FILES) return [];
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (stats.isDirectory()) {
          ...
          const children = await this.buildFileStructure(
            itemPath,
            basePath,
            depth + 1,
            collected,
          ...
          if (collected.count >= this.MAX_FILES) continue;
          collected.count++;
