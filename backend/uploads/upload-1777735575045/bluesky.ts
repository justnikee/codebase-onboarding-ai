...
/**
 * Post a standalone top-level post (no parent thread).
 * Automatically detects @mentions and URLs and adds rich-text facets.
 */
export async function postStandalone(text: string): Promise<ReplyRef> {
  const rt = new RichText({ text });
  await rt.detectFacets(agent); // builds mention + link facets
