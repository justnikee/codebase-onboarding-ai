...
const HELP_TEXT =
  `🎮 TrendArena Bot — Predict Bluesky virality!\n\n` +
  `Reply to any post + one of these commands:\n` +
  `• "@${BOT_HANDLE} viral" → predict it BLOWS UP 🚀\n` +
  `• "@${BOT_HANDLE} flop"  → predict it DOESN'T 💤\n` +
  `• "@${BOT_HANDLE} stats" → see your score 📊\n\n` +
  `Results in ${RESOLVE_MINUTES} mins. Leaderboard: ${APP_URL}/leaderboard`;
...
export async function handleMention(post: JetstreamPost): Promise<void> {
  ...
  // ── help ──────────────────────────────────────────────────────────────
  if (cmd.type === "help") {
    await postReply(HELP_TEXT, parentRef, rootRef);
    return;
  }
  ...
  if (cmd.type === "viral") {
    ...
    const confirmText =
      ...
      `Current likes: ${metrics.like_count}\n` +
      `Result in ${RESOLVE_MINUTES} mins — if you're right, +${process.env.BOT_POINTS_CORRECT ?? 100} pts!\n\n` +
      `🏆 ${APP_URL}/leaderboard`;
